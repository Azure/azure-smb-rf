...
.DESCRIPTION
    ...
    associates the target subscription. The operator must have permission to
    create management groups under the tenant root.
...
.NOTES
    Version: 1.0
    Author: SMB Ready Foundations
    Prerequisites: Tenant-root management group creation permission
#>
...
if ($LASTEXITCODE -ne 0 -or -not $tenantRootMgId) {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Host "        Ask a tenant administrator to grant management group access." -ForegroundColor Yellow
        exit 1
    }
}
